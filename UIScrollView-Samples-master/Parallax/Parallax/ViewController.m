...
- (void)setupPages
{
    ...
    for (int i = 0; i < pages; ++i) {
        ...
        [self testSizeFittingWithLabel:title];
//        [title sizeToFit];
...
// 验证sizetofit 和sizethatfit
- (void) testSizeFittingWithLabel:(UILabel *)label{
    //使用sizetofit
//    [self useSizeToFitWithLabel:label];
    //使用sizethatfits CGSizeZero
    [self useSizeThatFitsZeroWithLabel:label];
    //使用sizethatfits 任意
//    [self useSizeThatFitsCustomWithLabel:label];
}

- (void) useSizeToFitWithLabel:(UILabel *)label{
    [label sizeToFit];
}
