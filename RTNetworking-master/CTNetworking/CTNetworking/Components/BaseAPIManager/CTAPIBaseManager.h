...
//api回调
/// api回调的接口，相关VC要实现这个接口。这个是以delegate的形式产生的，一般我们用的是 block
...
/// 这里 VM 和 reformer 的不同在于，VM保存了view使用的全部数据，一个 View 对应一个 VM，VM 中可以包含更小的 VM，数据是由 manager 全部传给 VM，然后再由 VM 将处理好的数据给 View； reformer 不保存数据，而是在 VC 中通过 reformer 将数据取出，然后 VC 再将这个数据交付给 View
/// 又发现一点，casa 是将数据放在 manager 中保存的，所以用 reformer 修改。但是 VM 是会保存数据的，那么就不需要在 manager 中保存数据了吧，不然就会产生两个数据源
/// 所以我觉得还是 model + reform 的形式吧。不要单用 reformer。还是强类型比较好。
/*************************************************************************************************/
...
/// base 里的是内部拦截方法，interceptor 是外部拦截，外部拦截一般是 VC 操作的，内部拦截是 APImanager 操作的。拦截的时候，由内部拦截调用外部拦截
/// 一般情况下子类 manager 不需要重写这个方法，如果重写了就一定要调用 super，否则无法调用外部拦截。
